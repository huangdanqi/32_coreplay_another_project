...
# -*- coding: utf-8 -*-
"""
Enhanced test script to generate proper diary entries without <think tags.
This script fixes the LLM output to generate clean diary content.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('clean_diary_test.log'),
...
async def test_clean_diary_generation():
    """Test clean diary generation without <think tags."""
    print("=== Clean Diary Entry Generation Test ===")
    print("=" * 60)
    ...
    try:
        ...
        # Initialize prompt configuration with explicit instructions to avoid <think tags
...
情感标签（从：生气愤怒、悲伤难过、担忧、焦虑忧愁、惊讶震惊、好奇、羞愧、平静、开心快乐、兴奋激动中选择）：""",
        ...
        with open('clean_diary_results.json', 'w', encoding='utf-8') as f:
            json.dump(all_results, f, indent=2, ensure_ascii=False)
        
        print(f"\n💾 清理后结果已保存到: clean_diary_results.json")
        ...
        print(f"❌ 测试过程中出现错误: {e}")
        logger.error(f"清理测试失败: {e}", exc_info=True)
...
def main():
    ...
    # Run the clean test
    asyncio.run(test_clean_diary_generation())
    
    print("\n🎯 所有测试完成!")
    print("查看 clean_diary_results.json 获取清理后的结果")
